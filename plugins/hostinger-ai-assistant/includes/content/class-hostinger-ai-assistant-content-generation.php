...
class Hostinger_Ai_Assistant_Content_Generation {
	...
	public function create_post( string $post_type, string $title, string $content, int $category_id, array $tags_ids, int $featured_image_id = 0, string $post_status = 'draft' ): int {
		...
		if ( use_block_editor_for_post_type( $post_type ) && !empty( $content ) ) {
		...
		$this->amplitude->ai_content_saved( $post_type, $post_id );
		...
		if( $post_status === 'publish' ) {
			$this->amplitude->ai_content_published( $post_type, $post_id );
		}
	...
	public function process_post_action( string $action ): void {
		...
		try {
			...
			if ( $this->helper->post_type_supports_featured_image( $post_type ) && ( $thumbnail_image || $content_image ) ) {
				...
				if ( ! empty( get_object_vars( $image_data ) ) ) {
					...
					$image = wp_get_attachment_image( $featured_image_id, 'full', "", array( "class" => "hts-content-image" ) );
				}
			}

			if ( ! empty( $image ) && $content_image ) {
				$content = $image . $content;
			}

			$featured_image_id = ( ! $thumbnail_image ) ? 0 : ( $featured_image_id ?? 0 );
